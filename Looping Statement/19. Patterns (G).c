//Write a Program to Print Square Series.
...
int i, j, low = 0, top = 9, n = 1;: Variables i and j are loop counters, 
low represents the starting index of the current spiral,
top represents the ending index of the current spiral, and n is the number to be placed in the array.

Loop Execution:

The outer loop for (i = 0; i < 5; i++, low++, top--) iterates 5 times because there are 5 spirals in a 10x10 array.
Each iteration fills one spiral from the outermost to the innermost layer.
